...
// 判断是否是res.json文件
    ...
    if
    (fs.existsSync(resJson) && resJson.indexOf(
            "res.json"
        ) > -
            1
    ) {
        ...
        var
            defaultJson = fs.readFileSync(resJson,
                "utf-8"
            );
...
// 解析res.json文件内容
        ...
        var
            defaultObject = JSON.parse(defaultJson);


        var
            groups = defaultObject.groups;
//组


        var
            resources = defaultObject.resources;
//资源
            ...
            var
                urls = newKeys.map(
                    function
                        ...
                        return
                        path.join(pathNor, resourcesHash_1[key]);
...
// 根据tmproject写入文件
            ...
            (urls.length >
                ...
                fs.writeFileSync(path.join(targetDir, tmproject[
                        "projectName"
                        ] +
                    ".tmproject"
                ), JSON.stringify(tmproject));
